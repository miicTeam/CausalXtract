...
- Matlab is installed (needed for the cell feature module) \
\ \ Note that you can skip the cell feature part if you don't have Matlab \
...
- the image sequences from "http://???" \
\ \ have been downloaded in \
  ??? the demo folder ??? \
  ??? an "ImagesSequences" subfolder inside the "Demo" folder ??? \
  ??? an "ImagesSequences" folder next to the "Demo" folder ??? \
\
...
# if ( ! dir.exists ("./???") )
#  stop ("Sequences of images not found")
...
Here, we execute the cell feature module to perform segmentation & tracking, followed by the feature extraction. \
...
# +load(matlab))
# library(matlab)
# https://www.youtube.com/watch?v=QcodKT82Rtw
# MATLAB COMMANDS
# cat ("Cell feature module\n")
```
