...
-   the **MATLAB_DATA.rar** file from [*https://doi.org/10.5281/zenodo.7755700*](https://doi.org/10.5281/zenodo.7755700){.uri} has been downloaded and extracted in the **CausalXtract/MATLAB_DATA** folder. The **ROI MAT.rar** and **ROI_TU MAT.rar** files have to be extracted as well inside the MATLAB_DATA folder.
...
An alternate way to execute the causal discovery module is to use the online server at <https://miic.curie.fr/workbench_timeseries.php>, providing the same parameters and inputs: dataset file, state_order file, activate the temporal mode, set the maximum number of nodes to 200, activate the skeleton cut by setting the number of shuffles to 100 and confidence cut to 0.01, then set orientation confidence cut to 0.01.
